<template>
  <v-row>
    <div id="messages" class="display-area">
      <v-progress-circular
      ...
      <v-row
        ...
        v-else
        :key="index"
        class="display-area__message-container"
        :class="
          isMe(message.uid) ? 'display-area__message-container--right' : ''
        "
      >
        <v-col lg="4" md="3" sm="12">
          <v-card class="display-area__message-card">
            <v-row>
              <v-col cols="3">
                <img
                  class="display-area__profile-avi"
                  :src="message.photoURL"
                />
              </v-col>
              <v-col
                ><span class="display-area__name">{{
                  message.displayName
                }}</span></v-col
              >
              <v-col
                ><span class="display-area__date">{{
                  convertDate(message.dateAdded.seconds)
                }}</span></v-col
              >
            </v-row>
            <v-row>
